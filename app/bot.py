...
async def start(message: Message) -> None:
    ...
    text = (
        'Здравствуйте, в данном боте Вы можете заполнить заявление на вступление в члены РОО "Белорусская '
        'федерация брейкинга", получить ссылку на оплату вступительного и/или членского взноса, или загрузить '
        "уже имеющийся платёжный документ подтверждающий факт оплаты."
    )
...
async def notify_admins(application: MemberApplication, payment: Payment, db: Session) -> None:
    ...
    text = (
        ...
        f"Паспорт №: {application.passport_number or '-'}\n"
        f"Паспорт выдан: {application.passport_issued_by or '-'}\n"
        f"Дата заявления: {application.statement_date or '-'}\n"
        f"Мать / представитель: {application.mother_full_name or '-'}\n"
        f"Мать: место работы, должность: {application.mother_workplace_position or '-'}\n"
        f"Отец / представитель: {application.father_full_name or '-'}\n"
        f"Отец: место работы, должность: {application.father_workplace_position or '-'}\n"
        f"\nОплата\n"
